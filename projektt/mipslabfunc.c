...
secCount = 0;
buttonCount=0;
projectileCount = 0;  //count how many proj
createProjectileCount = 0;
mapCount = 0;
createMapCount=0;
lives = 3;
randCount = 0;  //randomize clouds etc
gamecount = 0;  //counter to spawn enemies   NEW
number_of_enemies = 0; //counter fornumbers of enemies NEW
TIE1_alive = 0;
TIE2_alive = 0;
TIE3_alive = 0;
TIE4_alive = 0;
...
void spawn_enemies(int gametimer){
      if(number_of_enemies <=8){
        switch (gametimer) {
          case 1:
            spawn_enemy(110,4, TIE1,32);
            TIE1_alive =1;
            number_of_enemies++;
          break;
  /*        case 3:
            spawn_enemy(110,8, TIE2,32);
          break;
          case 6:
            spawn_enemy(110,14, TIE3,32);
          break;
          case 9:
            spawn_enemy(110,16, TIE4,32);
          break;
          case 12:
            spawn_enemy(110,20, TIE5,32);
          break;
*/        default:
          break;
          }
...
void move_enemies(number_of_enemies){ //NEW
        int i,j;
//      if (enemies_alive>0){
        if (TIE1_alive==1){
        for (j=0; j < 4;j++) {
...
//spawn an enemy
void spawn_enemy(int x, int y, int array[], int arrayLength){ //new ALL CLEAR
    int i;
    for (i = 0; i < arrayLength/2;i++) {
        set_coordinate((array[i]),(array[i+arrayLength/2]), game,0);
    }
    for (i = 0; i < arrayLength/2;i++){
...
void run_projectile(void){
    ...
    projectileCount++;
    if(createProjectileCount < 1){    //if zero create and projectile

        createProjectileCount = 1;
        if ((getbtns() & 0x1) == 1) {
            create_projectile(ship_placementX+7,ship_placementY+1,1);
        }
    }
...
void run_pause(void){
  if ((getbtns() & 0x8) ==8 && pause == 0){
    pause =1;
    while(pause==1){
      if ((getbtns() & 0x8) ==8 && pause == 1){
        pause=0;
      }
    }
  }
